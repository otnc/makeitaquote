...
One structural rule is worth knowing about: **`src/render/canvasFactory.ts`
is the only place `@napi-rs/canvas` is imported.** That keeps the native
binding out of the `makeitaquote/api` entry point, so the API client works
on platforms with no prebuilt binary. `check-build.js` enforces this both
statically and by loading the built entry and looking at what it pulled in.
...
left external, so this is caught rather than remembered — and, all else
equal, an ESM-only candidate is passed over for one that isn't.

See [INFO.md](INFO.md) for why each current dependency was picked and what
