...
describe('computeLayout · stacked', () => {
  ...
  it('ignores the avatar side', () => {
    const left = computeLayout(defineTheme({ layout: 'stacked', avatar: { position: 'left' } }))
    const right = computeLayout(defineTheme({ layout: 'stacked', avatar: { position: 'right' } }))
...
describe('gradientLine', () => {
  ...
  it('runs top to bottom when vertical', () => {
    const [x0, y0, x1, y1] = gradientLine(defineTheme({ layout: 'stacked' }))
  ...
  it('does not mirror a vertical gradient for a right avatar', () => {
    const theme = defineTheme({ layout: 'stacked', avatar: { position: 'right' } })
