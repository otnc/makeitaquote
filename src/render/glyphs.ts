 ...
 *
 * Latin is absent on purpose: everything has it, so it never changes what we do.
 */
 ...
 *
 * The requested family stays in front and the fallbacks go behind it. Skia resolves a stack per glyph, so the chosen font is still used for everything it has — measured on Vina Sans (Latin only) with Noto Sans JP behind it, `A` keeps Vina's width and `猫` takes Noto's.
 *
 * No attempt is made to detect whether the primary "covers" a script: a font missing a glyph does not reliably measure as tofu, so a probe based on width gets it wrong. Naming the fallbacks unconditionally costs nothing when they are not needed and is right when they are.
 */
export function coveringStack(request: string, text: string, fallbacks: readonly string[]): string {
  ...
  for (const family of fallbacks) {
    // Only families already registered — this is a rendering decision, not a reason to reach for the network.
