...
/**
 * Parses MFM — Misskey Flavoured Markup — into styled runs: bold, italic and
 * strikethrough rendered instead of stripped.
 *
 * Parsing goes through `mfm-js`, Misskey's own parser, rather than a local
 * approximation — see `stripMfm()` for why. `small`/`center` change font size
 * or layout rather than style, so (like headings elsewhere in this package)
 * they are left structural: their content passes through at the surrounding
 * style, undecorated.
 *
 * Kept exactly as `stripMfm()` keeps them — `:name:` custom emoji, `@user`/
 * `@user@host` mentions, `#hashtag` and bare URLs are all plain runs.
 */
 ...
 *
 * MFM has enough corners — `$[fn …]` re-parses its contents so functions
 * nest, `<center>` is a block that only counts at the start of a line, code
 * and maths are verbatim — that matching the reference implementation is
 * worth the dependency. A thin wrapper over `parseMfm()`, discarding the
 * style it tracks.
 *
...
function toStyledNode(node: MfmNode, style: TextStyle | undefined): StyledRun[] {
  switch (node.type) {
    ...
    // `small`/`center` change size or layout, not style (deferred — see the
    // module doc comment); `plain`, `quote` and every `$[fn …]` are otherwise
